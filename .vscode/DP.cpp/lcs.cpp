...
#include<vector>
using namespace std;
void printlis(string s1 , string s2 , int dp [100][100], int n , int m , vector <char>&ans){
	if (n ==0  and m==0)
	{
	...
	if (s1[n-1]==s2[m-1])
	{
		...
		if (dp[n-1][m]==dp[n][m])
		{
			/* code */printlis(s1,s2,dp,n-1,m,ans);
		}
		if (dp[n][m-1]==dp[n][m])
		{
			/* code */printlis(s1,s2,dp,n,m-1,ans);
		}
